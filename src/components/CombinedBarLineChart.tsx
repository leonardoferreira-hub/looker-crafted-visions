      ...
      <div className="bg-card/95 backdrop-blur-sm border border-border/50 rounded-lg p-3 shadow-lg">
        ...
        {payload.map((entry, index) => (
          <p key={index} className="text-sm" style={{ color: entry.color }}>
            ...
            {entry.dataKey === 'acumulado2026' && `Acumulado 2026: ${entry.value} operações`}
          </p>
        ))}
      </div>
    );
...
export function CombinedBarLineChart({ data, endDate, comparisonEndDate }: { data: any[]; endDate?: Date; comparisonEndDate?: Date }) {
  // Filter data to show 2026 only until the end date from filter period
  const endMonth2026 = endDate ? endDate.getMonth() + 1 : new Date().getMonth() + 1;
  
  // Filter data to show 2025 only until the comparison end date
  const endMonth2025 = comparisonEndDate ? comparisonEndDate.getMonth() + 1 : 12; // Default to December if no comparison filter
  ...
  const filteredData = data.map((item, index) => {
    ...
    // Filter 2026 data based on period filter
    if (monthIndex > endMonth2026) {
      filteredItem.acumulado2026 = undefined;
    }
    
    // Filter 2025 data based on comparison filter
